...
struct ThreadsBackend end
const THREADS_BACKEND = ThreadsBackend()
...
# Fast path implementations for parallel operations
function parallel_for(N::I, f::F, x...) where {I <: Integer, F <: Function}
...
function parallel_for((M, N)::Tuple{I, I}, f::F, x...) where {I <: Integer, F <: Function}
    if default_backend() isa ThreadsBackend
        ...
        parallel_for(default_backend(), (M, N), f, x...)
    end
end
...
function parallel_for((L, M, N)::Tuple{I, I, I}, f::F, x...) where {I <: Integer, F <: Function}
    if default_backend() isa ThreadsBackend
        ...
        parallel_for(default_backend(), (L, M, N), f, x...)
    end
end

# Backend-specific implementations (for GPUs)
function parallel_for(backend::Any, N::I, f::F, x...) where {I <: Integer, F <: Function}
    @maybe_threaded for i in 1:N
        f(i, x...)
    end
end

function parallel_for(backend::Any, (M, N)::Tuple{I, I}, f::F, x...) where {I <: Integer, F <: Function}
    @maybe_threaded for j in 1:N
        for i in 1:M
            f(i, j, x...)
        end
...
function parallel_for(backend::Any, (L, M, N)::Tuple{I, I, I}, f::F, x...) where {I <: Integer, F <: Function}
    @maybe_threaded for k in 1:N
        for j in 1:M
            for i in 1:L
                f(i, j, k, x...)
...
# Optimized parallel_reduce implementations
function parallel_reduce(N::Integer, op, f::Function, x...; init)
    if default_backend() isa ThreadsBackend
        ret = init
        chunk_size = max(N ÷ (4 * Threads.nthreads()), 1)
        tmp = fill(init, Threads.nthreads())
        @maybe_threaded for i in 1:chunk_size:N
            tid = Threads.threadid()
            for j in i:min(i+chunk_size-1, N)
                tmp[tid] = op.(tmp[tid], f(j, x...))
            end
        end
        for i in 1:Threads.nthreads()
            ret = op.(ret, tmp[i])
...
function parallel_reduce((M, N)::Tuple{Integer, Integer}, op, f::Function, x...; init)
    if default_backend() isa ThreadsBackend
        ...
        chunk_size = max(N ÷ (4 * Threads.nthreads()), 1)
        tmp = fill(init, Threads.nthreads())
        @maybe_threaded for j in 1:chunk_size:N
            tid = Threads.threadid()
            for k in j:min(j+chunk_size-1, N)
                for i in 1:M
                    tmp[tid] = op.(tmp[tid], f(i, k, x...))
                end
            end
        end
        for i in 1:Threads.nthreads()
...
# Default reductions with + operator
function parallel_reduce(N::Integer, f::Function, x...)
    parallel_reduce(N, +, f, x...; init = zero(Float64))
end

function parallel_reduce((M, N)::Tuple{Integer, Integer}, f::Function, x...)
    parallel_reduce((M, N), +, f, x...; init = zero(Float64))
end
...
get_backend(::Val{:threads}) = THREADS_BACKEND
